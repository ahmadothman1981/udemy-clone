<template>
  <div class="bg-slate-50 dark:bg-slate-900 min-h-screen flex font-inter transition-colors duration-300">
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
      <AdminHeader :title="$t('admin.reviews.title')" @logout="handleLogout" />
      ...
      <div class="flex-1 overflow-auto p-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 mb-6 flex flex-wrap items-center gap-4 transition-colors">
          <select v-model="ratingFilter" @change="fetchReviews" class="px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-800 dark:text-white transition-colors">
            ...
            <option v-for="r in [5,4,3,2,1]" :key="r" :value="r">{{ r }} {{ $t('admin.reviews.stars') }}</option>
          </select>
          <input v-model="courseSearch" @input="debouncedSearch" :placeholder="$t('admin.reviews.filter_placeholder')" class="px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 rounded-lg w-48 text-slate-800 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-purple-500 transition-colors" />
        </div>
              ...
              <div class="flex justify-between items-start">
                <div class="flex gap-4">
                  ...
                  <div>
                    ...
                    <div class="text-sm text-slate-500 dark:text-slate-400 mb-2 transition-colors">{{ $t('admin.reviews.on') }} "{{ review.course?.title }}"</div>
                ...
                <button @click="removeReview(review)" class="p-2 text-red-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" :title="$t('admin.reviews.remove')">
                  <Trash2 class="w-4 h-4" />
                </button>
              </div>
            </div>
            <div v-if="reviews.length === 0" class="p-12 text-center text-slate-400 dark:text-slate-500 transition-colors">{{ $t('admin.reviews.no_reviews') }}</div>
          </div>
          <!-- Pagination -->
          <div class="p-4 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center transition-colors">
            <span class="text-sm text-slate-500 dark:text-slate-400">{{ $t('admin.reviews.showing', {from: pagination.from, to: pagination.to, total: pagination.total}) }}</span>
            <div class="flex gap-2">
              <button @click="changePage(pagination.current_page - 1)" :disabled="pagination.current_page === 1" class="px-3 py-1 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-600 dark:text-slate-300 disabled:opacity-50 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">{{ $t('admin.reviews.previous') }}</button>
              <button @click="changePage(pagination.current_page + 1)" :disabled="pagination.current_page === pagination.last_page" class="px-3 py-1 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-600 dark:text-slate-300 disabled:opacity-50 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">{{ $t('admin.reviews.next') }}</button>
            </div>
          </div>
        </div>
      </div>
...
const removeReview = async (review) => {
  const result = await confirmDelete(t('admin.reviews.review_item'));
  if (!result.isConfirmed) return;
  await axios.delete(`/api/admin/reviews/${review.id}`);
  showSuccess(t('admin.reviews.removed'));
  fetchReviews();
};
