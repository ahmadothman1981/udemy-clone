  ...
  <div class="bg-slate-50 min-h-screen flex font-inter">
      ...
      <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div class="flex-1 overflow-auto p-8">
            <!-- Tabs -->
            <div class="flex space-x-1 rounded-xl bg-slate-200 p-1 mb-6 max-w-2xl">
              <button 
                v-for="tab in tabs" 
                :key="tab.id"
            ...
            <div v-if="activeTab === 'categories'" class="space-y-6">
              <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-100 flex justify-between items-center">
                  ...
                  <button @click="openCategoryModal()" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 flex items-center gap-2">
                  ...
                  <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-100">
                      <tr v-for="cat in categories" :key="cat.id" class="hover:bg-slate-50">
                        ...
                        <td class="px-6 py-4 text-sm text-slate-500">{{ cat.courses_count || 0 }}</td>
                        <td class="px-6 py-4 text-right">
                          <button @click="openCategoryModal(cat)" class="p-2 text-slate-400 hover:text-blue-600 rounded-lg"><Pencil class="w-4 h-4" /></button>
                          <button @click="deleteCategory(cat)" class="p-2 text-slate-400 hover:text-red-600 rounded-lg"><Trash2 class="w-4 h-4" /></button>
                        </td>
                      </tr>
                    </tbody>
                  </table>
            ...
            <div v-if="activeTab === 'payment'" class="space-y-6">
              <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <h2 class="text-lg font-bold text-slate-800 mb-4">Payment Settings</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Platform Commission (%)</label>
                    <input v-model="paymentSettings.platform_commission" type="number" min="0" max="100" class="w-full px-4 py-2 border rounded-lg" />
                    <p class="text-xs text-slate-500 mt-1">Percentage of each sale retained by the platform</p>
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Minimum Payout ($)</label>
                    <input v-model="paymentSettings.payout_minimum" type="number" min="0" class="w-full px-4 py-2 border rounded-lg" />
                    <p class="text-xs text-slate-500 mt-1">Minimum balance required for instructor payouts</p>
                  </div>
                </div>
                <div class="mt-6 flex justify-end">
                  <button @click="savePaymentSettings" class="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Save Payment Settings</button>
                </div>
              </div>
            </div>
            ...
            <div v-if="activeTab === 'security'" class="space-y-6">
              <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <h2 class="text-lg font-bold text-slate-800 mb-4">Security Settings</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Minimum Password Length</label>
                    <input v-model="securitySettings.password_min_length" type="number" min="6" max="32" class="w-full px-4 py-2 border rounded-lg" />
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Session Timeout (minutes)</label>
                    <input v-model="securitySettings.session_timeout" type="number" min="5" max="1440" class="w-full px-4 py-2 border rounded-lg" />
                  </div>
                </div>
                <div class="mt-6 flex justify-end">
                  <button @click="saveSecuritySettings" class="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Save Security Settings</button>
                </div>
              </div>
            </div>
            ...
            <div v-if="activeTab === 'localization'" class="space-y-6">
              <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                <h2 class="text-lg font-bold text-slate-800 mb-4">Localization Settings</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Default Language</label>
                    <select v-model="localizationSettings.default_language" class="w-full px-4 py-2 border rounded-lg">
                  ...
                  <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Enabled Languages</label>
                    <div class="flex flex-wrap gap-2">
                      <label v-for="lang in ['en', 'ar', 'es', 'fr']" :key="lang" class="flex items-center gap-2">
                        <input type="checkbox" :value="lang" v-model="localizationSettings.enabled_languages" class="w-4 h-4" />
                        {{ lang.toUpperCase() }}
                      </label>
                    </div>
                  </div>
                ...
                <div class="mt-6 flex justify-end">
                  <button @click="saveLocalizationSettings" class="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">Save Localization Settings</button>
                </div>
              </div>
            </div>
        </div>
      ...
      <div v-if="categoryModalVisible" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="categoryModalVisible = false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
          <div class="p-6 border-b"><h3 class="text-lg font-bold">{{ editingCategory ? 'Edit' : 'Add' }} Category</h3></div>
          <div class="p-6 space-y-4">
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Name</label>
              <input v-model="categoryForm.name" class="w-full px-4 py-2 border rounded-lg" />
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Slug (auto-generated if empty)</label>
              <input v-model="categoryForm.slug" class="w-full px-4 py-2 border rounded-lg" placeholder="programming-basics" />
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Description</label>
              <textarea v-model="categoryForm.description" class="w-full px-4 py-2 border rounded-lg" rows="3"></textarea>
            </div>
          </div>
          <div class="p-4 bg-slate-50 flex justify-end gap-2">
            <button @click="categoryModalVisible = false" class="px-4 py-2 bg-slate-200 rounded-lg">Cancel</button>
            <button @click="saveCategory" class="px-4 py-2 bg-purple-600 text-white rounded-lg">Save</button>
          </div>
        </div>
      </div>
  </div>
